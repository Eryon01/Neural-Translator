...
# Function to transcribe audio using SpeechRecognition with caching
def transcribe_audio(audio_file):
    try:
        ...
        with sr.AudioFile(audio_file) as source:
            # Record the audio data from the entire file
...
def process_language(transcript_text, language_code, language_name):
    if not transcript_text or transcript_text.startswith("Error") or transcript_text.startswith(
...
with gr.Blocks() as demo:
    ...
    with gr.Tabs():
        ...
        #     ru_output = gr.Audio(ladwbel="Russian Audio", interactive=False)
...
if __name__ == "__main__":
    # No need to check for AssemblyAI API key
